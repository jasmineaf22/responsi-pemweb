...
<?php
if ($user['isAdmin'] != 1 || ($user['isAdmin'] == 1 && !isset($_GET['mov']))) {
  ...
  <body>
    ...
    <main>
      ...
      <div class="container">
        ...
        $movieId = $_GET['mov'];   
        ?>
        <form action="../function/castProcess.php?mov=<?= $movieId ?>" method="post" enctype="multipart/form-data">
          <label for="name-cast">Name Actor/Actress</label>
          <input type="text" name="cast" id="name-cast" autocomplete="off" placeholder="Name Actor/Actress" required />
          <label for="name-chara">Name Character Movie</label>
          <input type="text" name="chara" id="name-chara" autocomplete="off" placeholder="Name Character Movie" required/>
          <label for="file">Image Actor/Actress</label>
          <input type="file" name="file" id="file" accept="image/*" require/>
          <label for="file">
            <div class="label">
                <svg width="40" viewBox="0 0 50 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
